<?php
if (!class_exists('Updatr_v1p0_Plugin_Ui', false)) :
    /**
     * Additional UI elements for plugins.
     */
    class Updatr_v1p0_Plugin_Ui
    {
        private $updateChecker;
        private $manualCheckErrorTransient = '';

        /**
         * @param Updatr_v1p0_Plugin_UpdateChecker $updateChecker
         */
         ...
         *
         * You can change the link text by using the "updatr_view_details_link-$slug" filter.
         * Returning an empty string from the filter will disable the link.
         *
         * You can change the position of the link using the
         * "updatr_view_details_link_position-$slug" filter.
         ...
         *
         * You can change the link text by using the "updatr_manual_check_link-$slug" filter.
         * Returning an empty string from the filter will disable the link.
         *
         * @param array $pluginMeta Array of meta links.
         * @param string $pluginFile
        ...
        public function addCheckForUpdatesLink($pluginMeta, $pluginFile)
        {
            if ($this->isMyPluginFile($pluginFile)) {
                $linkUrl = wp_nonce_url(
                    add_query_arg(
                        array(
                            'updatr_check_for_updates' => 1,
                            'updatr_slug'              => $this->updateChecker->slug,
                        ),
                        self_admin_url('plugins.php')
                    ),
                    'updatr_check_for_updates'
                );
        ...
        {
            $shouldCheck =
                isset($_GET['updatr_check_for_updates'], $_GET['updatr_slug'])
                && $_GET['updatr_slug'] == $this->updateChecker->slug
                && check_admin_referer('updatr_check_for_updates');
            ...
            if ($shouldCheck) {
                ...
                if (($update === null) && !empty($this->lastRequestApiErrors)) {
                    //Some errors are not critical. For example, if WP-Updatr tries to retrieve the readme.txt
                    ...
                    $questionableErrorCodes = array(
                        'updatr-github-http-error',
                        'updatr-gitlab-http-error',
                        'updatr-bitbucket-http-error',
                    );
                ...
                wp_redirect(add_query_arg(
                    array(
                        'updatr_update_check_result' => $status,
                        'updatr_slug'                => $this->updateChecker->slug,
                    ),
                    self_admin_url('plugins.php')
                ));
         ...
         *
         * You can change the result message by using the "updatr_manual_check_message-$slug" filter.
         */
        public function displayManualCheckResult()
        {
            if (isset($_GET['updatr_update_check_result'], $_GET['updatr_slug']) && ($_GET['updatr_slug'] == $this->updateChecker->slug)) {
                $status = strval($_GET['updatr_update_check_result']);
