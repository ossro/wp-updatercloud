<?php
if (!class_exists('UpdaterCloud_v1p0_Plugin_Ui', false)) :
    /**
     * Additional UI elements for plugins.
     */
    class UpdaterCloud_v1p0_Plugin_Ui
    {
        private $updateChecker;
        private $manualCheckErrorTransient = '';

        /**
         * @param UpdaterCloud_v1p0_Plugin_UpdateChecker $updateChecker
         */
         ...
         *
         * You can change the link text by using the "updatercloud_view_details_link-$slug" filter.
         * Returning an empty string from the filter will disable the link.
         *
         * You can change the position of the link using the
         * "updatercloud_view_details_link_position-$slug" filter.
         ...
         *
         * You can change the link text by using the "updatercloud_manual_check_link-$slug" filter.
         * Returning an empty string from the filter will disable the link.
         *
         * @param array $pluginMeta Array of meta links.
         * @param string $pluginFile
        ...
        public function addCheckForUpdatesLink($pluginMeta, $pluginFile)
        {
            if ($this->isMyPluginFile($pluginFile)) {
                $linkUrl = wp_nonce_url(
                    add_query_arg(
                        array(
                            'updatercloud_check_for_updates' => 1,
                            'updatercloud_slug'              => $this->updateChecker->slug,
                        ),
                        self_admin_url('plugins.php')
                    ),
                    'updatercloud_check_for_updates'
                );
        ...
        {
            $shouldCheck =
                isset($_GET['updatercloud_check_for_updates'], $_GET['updatercloud_slug'])
                && $_GET['updatercloud_slug'] == $this->updateChecker->slug
                && check_admin_referer('updatercloud_check_for_updates');
            ...
            if ($shouldCheck) {
                ...
                if (($update === null) && !empty($this->lastRequestApiErrors)) {
                    //Some errors are not critical. For example, if WP-UpdaterCloud tries to retrieve the readme.txt
                    ...
                    $questionableErrorCodes = array(
                        'updatercloud-github-http-error',
                        'updatercloud-gitlab-http-error',
                        'updatercloud-bitbucket-http-error',
                    );
                ...
                wp_redirect(add_query_arg(
                    array(
                        'updatercloud_update_check_result' => $status,
                        'updatercloud_slug'                => $this->updateChecker->slug,
                    ),
                    self_admin_url('plugins.php')
                ));
         ...
         *
         * You can change the result message by using the "updatercloud_manual_check_message-$slug" filter.
         */
        public function displayManualCheckResult()
        {
            if (isset($_GET['updatercloud_update_check_result'], $_GET['updatercloud_slug']) && ($_GET['updatercloud_slug'] == $this->updateChecker->slug)) {
                $status = strval($_GET['updatercloud_update_check_result']);
