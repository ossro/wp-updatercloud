...
if (!class_exists('UpdaterCloud_v1p0_Metadata', false)) :
     ...
     * @access public
     */
    abstract class UpdaterCloud_v1p0_Metadata
    {
        /**
        ...
        protected static function createFromJson($json, $target)
        {
            ...
            if (empty($apiResponse) || !is_object($apiResponse)) {
                $errorMessage = "Failed to parse update metadata.";
                do_action('updatercloud_api_error', new WP_Error('updatercloud-invalid-json', $errorMessage));
                trigger_error($errorMessage, E_USER_NOTICE);
                return false;
            }

            if (! $apiResponse->update ?? null) {
                $errorMessage = "Failed to parse update metadata.";
                do_action('updatercloud_api_error', new WP_Error('updatercloud-invalid-json', $errorMessage));
                trigger_error($errorMessage, E_USER_NOTICE);
                return false;
            ...
            if (is_wp_error($valid)) {
                do_action('updatercloud_api_error', $valid);
                trigger_error($valid->get_error_message(), E_USER_NOTICE);
                return false;
            }

            foreach (get_object_vars($apiResponse->update) as $key => $value) {
                $target->$key = $value;
            }
