...
if (!class_exists('UpdaterCloud_v1p0_DebugBar_Panel', false) && class_exists('Debug_Bar_Panel', false)) :

    class UpdaterCloud_v1p0_DebugBar_Panel extends Debug_Bar_Panel
    {
        /** @var UpdaterCloud_v1p0_UpdateChecker */
        protected $updateChecker;

        private $responseBox = '<div class="updatercloud-ajax-response" style="display: none;"></div>';

        public function __construct($updateChecker)
        {
            $this->updateChecker = $updateChecker;
            $title = sprintf(
                '<span class="updatercloud-debug-menu-link-%s">WP-UpdaterCloud (%s)</span>',
        ...
        {
            printf(
                '<div class="updatercloud-debug-bar-panel-v1" id="%1$s" data-slug="%2$s" data-uid="%3$s" data-nonce="%4$s">',
                esc_attr($this->updateChecker->getUniqueName('debug-bar-panel')),
                esc_attr($this->updateChecker->slug),
                esc_attr($this->updateChecker->getUniqueName('uid')),
                esc_attr(wp_create_nonce('updatercloud-ajax'))
            );
        ...
        private function displayConfiguration()
        {
            ...
            echo '<table class="updatercloud-debug-data">';
